...
const editarDocente = ((docente)=>{
    const indexOfDocente = docentes.indexOf(docentes.find((don)=>don.id === docente.id ));
    if(indexOfDocente >= 0){
...
const eliminarDocente = ((id)=>{
    const indexOfDocente = docentes.indexOf(docentes.find((don)=> don.id === id ));
